...
def six_class_label(label,num_clss):
...
class DataSetFactory:
    ...
    def __init__(self, NUM_CLASSES=6):
        ...
        with open('./fer2013/fer2013.csv', 'r') as csvin:
            ...
            for row in data:
                ...
                if row[-1] == 'Training':
                    emotions.append(six_class_label(int(row[0]),self.num_classes))
                    images.append(Image.fromarray(face))
                elif row[-1] == "PrivateTest":
                    private_emotions.append(six_class_label(int(row[0]),self.num_classes))
                    private_images.append(Image.fromarray(face))
                elif row[-1] == "PublicTest":
                    public_emotions.append(six_class_label(int(row[0]),self.num_classes))
...
def main():
    ...
    validation_loader = {
        'private': DataLoader(factory.private, batch_size=batch_size, shuffle=True),
        'public': DataLoader(factory.public, batch_size=batch_size, shuffle=True)
    }
    ...
    for epoch in range(epochs):
        ...
        writer.add_scalar('Total loss', total_train_loss / (i + 1), i)
